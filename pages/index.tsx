...
export default function Home() {
  ...
  const trendingMovies = [...movies].sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()).slice(0, 6);
  ...
  if (typeof window !== 'undefined') {
    window.addEventListener('scroll', () => setShowBackToTop(window.scrollY > 500));
  }
    ...
    <>
      ...
      <nav className="fixed top-0 left-0 right-0 z-40 bg-cinema-black/50 backdrop-blur-md border-b border-white/10">
        <div className="container mx-auto px-6 py-4 flex items-center justify-between">
          <motion.h1 className="font-heading text-3xl text-cinema-accent" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            CINEREVIEW
          </motion.h1>
